...
@section('content')
  <section class="section product-header no-mb">
    <form id="silkFilterForm" action="" data-ajaxurl="{{ $ajax_url_listing }}" method="GET">
      <div class="container">
        <div class="section-header is-search">
          <h1 class="title h2 text-center">{{ $search_header->title }}</h1>
          @if (is_search() && !empty($search_header->result_text) )
            <div class="content text-center">
              <p><em>{!! $search_header->result_text !!}</em></p>
            </div>
          @endif
        </div>
        ...
        @php $count = 1 @endphp
          @while (have_posts()) @php(the_post())
            ...
            @include('partials.product-item', [
              'product' => TaxonomySilk_category::get_product(get_the_ID()),
              'imageSize' => $imageSize,
              'post' => get_post() 
            ])
            @php($count++)
